...
public class CLOP_1872 {

	@Test
	public void CLOP_1872_Test() throws Exception {
		...
		// Revise the Learning Objective and replace it with another one.
		...
		// Validate the edited Learning Objective value.
		rowCount=Chrome_Driver.findElements(By.xpath("//*[@id='metadatatable']/tbody/tr")).size();
		for (int i=1;i<=rowCount;i++) {
			String sValue = null;
			sValue = Chrome_Driver.findElement(By.xpath("//*[@id='metadatatable']/tbody/tr["+i+"]/td[1]")).getText();		
			if (sValue.equalsIgnoreCase("Learning Objective")) {
				Assert.assertEquals("RM LO for standards -- LS 2 - Subject - Science B - Analyzing W - II Short term Grade 6", Chrome_Driver.findElement(By.xpath("//*[@id='metadatatable']/tbody/tr["+i+"]/td[2]")).getText());
			}
	    }
